...
import com.itsse.backend.service.SignService;
...
public class SignController {
    ...
    public SignInResultDto signIn(@RequestBody UserDto user) throws RuntimeException {
        ...
        if (signInResultDto.getCode() == 0) {
            LOGGER.info("[signIn] 정상적으로 로그인되었습니다. id : {}, token : {}", user.getId(),
    ...
    public SignUpResultDto signUp(@RequestBody UserDto user) {
        //
        LOGGER.info("[signUp] 회원가입을 수행합니다. id : {}, password : ****, name : {}, role : {}", user.getId(),
            user.getName(), user.getRole());
        SignUpResultDto signUpResultDto = signService.signUp(user.getId(), user.getPassword(), user.getName(), user.getRole());

        LOGGER.info("[signUp] 회원가입을 완료했습니다. id : {}", user.getId());
